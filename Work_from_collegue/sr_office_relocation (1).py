...
class SROfficeRelocationSolver:
    ...
    def greedy_solve(self, objective: str = 'total_distance', 
        ...
        for sr in range(self.n):
            ...
            y[closest_brick] = 1
            office_locations.append(int(closest_brick))  # Convert to Python int
        ...
        # Calculate metrics
        metrics = {
            'total_distance': float(self.calculate_objective_total_distance(x, y)),
            'workload_max': float(self.calculate_workload_max(x, y)),
            'displaced_offices': int(self.calculate_displaced_offices(y)),
            'office_locations': [int(loc) for loc in office_locations]
        }
    ...
    def save_solution(self, x: np.ndarray, y: np.ndarray, metrics: Dict, 
        ...
        solution = {
            'metrics': metrics,
            'problem_size': {'bricks': int(self.m), 'SRs': int(self.n)}
        }
...
def generate_reports(solver, x, y, metrics):
    ...
    for office_brick in office_locations:
        ...
        office_details.append({
            'SR': int(sr + 1),
            'Office_Brick': int(office_brick),
            'Num_Assigned': int(len(assigned_bricks)),
            'Workload': float(office_workload),
            'Avg_Distance': float(avg_distance),
            'X': float(solver.coords[office_brick, 0]),
            'Y': float(solver.coords[office_brick, 1])
        })
    ...
    for i in range(solver.m):
        ...
        brick_details.append({
            'Brick': int(i),
            'SR': int(solver.brick_zones[i] + 1),
            'Workload': float(solver.brick_workload[i]),
            'Office': int(assigned_office),
            'Distance': float(solver.distance_matrix[i, assigned_office])
        })
